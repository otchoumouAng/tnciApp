...
// Couleurs Industrielles Spécifiques pour les statuts
const STATUS_COLORS = {
  success: '#2E7D32', // Vert Industriel Profond
  warning: '#FFA000', // Ambre (Plus doux que l'orange sécurité #FF8F00)
  error: '#C62828'    // Rouge Alerte
};
...
const DeclarationPaletteScreen = () => {
  ...
  const fetchDeclaredPalettes = async () => {
    ...
    try {
      ...
      setError("Impossible de charger la liste des palettes. Veuillez réessayer plus tard.");
      setDeclaredPalettes([]);
    } finally {
      setLoading(false);
    }
  ...
  // Helper pour afficher le modal de résultat
  ...
  const handleBarCodeScanned = async ({ data }: { data: string }) => {
    ...
    try {
      ...
      if (palette.statut === 'DC') {
        // Remplacement de Alert.alert par le modal High-Tech Warning
        showResult(
          'warning', 
          'Déjà Déclarée', 
          `La palette N° ${palette.numero} est déjà enregistrée dans le système.`
        );
      ...
      // Remplacement de Alert.alert par le modal High-Tech Error
      showResult(
        'error', 
        'Scan Échoué', 
        "Palette introuvable ou QR code invalide. Vérifiez les données."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleValidation = async () => {
    ...
    try {
      ...
      // On ferme le modal de confirmation avant d'ouvrir le succès
      setScannedPalette(null);
      
      // Remplacement de Alert.alert par le modal High-Tech Success
      setTimeout(() => {
        showResult(
          'success', 
          'Déclaration Réussie', 
          `Palette ${scannedPalette.numero} validée et stockée.`
        );
      }, 300); // Petit délai pour fluidifier la transition des modals
      ...
      if (finalMessage.includes("déjà été déclarée")) {
        setTimeout(() => {
          showResult('warning', 'Déjà Déclarée', "Cette palette a déjà été traitée par un autre opérateur.");
        }, 300);
      } else {
        setTimeout(() => {
          showResult('error', 'Échec Déclaration', finalMessage);
        }, 300);
      }
    } finally {
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={localStyles.emptyContainer}>
            ...
            <Text style={Styles.emptyText}>Aucune palette déclarée aujourd'hui.</Text>
          </View>
        }
        contentContainerStyle={Styles.list}
        onRefresh={fetchDeclaredPalettes}
        refreshing={loading}
      />
    );
  };
      ...
      <View style={{ flex: 1, backgroundColor: 'black' }}>
        ...
        <View style={localStyles.overlay}>
          ...
          <View style={localStyles.overlayBottom}>
            <Text style={localStyles.scanInstruction}>ODMTEC - Visez le QR Code sur la palette</Text>
  ...
  return (
    <View style={Styles.container}>
      ...
      {/* --- MODAL DE RÉSULTAT (Succès / Echec / Warning) --- */}
      <Modal
        transparent={true}
        ...
        <View style={localStyles.modalOverlay}>
          <View style={localStyles.modalContent}>
            <View style={localStyles.modalHeader}>
              ...
              <Text style={localStyles.modalTitle}>Confirmer Déclaration</Text>
            </View>
      ...
      <View style={localStyles.headerContainer}>
        <Text style={localStyles.headerTitle}>Déclaration De Palette</Text>
        <Text style={localStyles.headerSubtitle}>Scanner les palettes en sortie de ligne</Text>
      </View>
      ...
      <View style={localStyles.actionContainer}>
        <TouchableOpacity style={localStyles.scanButton} onPress={() => setCameraVisible(true)}>
          ...
          <Text style={localStyles.scanButtonText}>SCANNER PALETTE</Text>
        </TouchableOpacity>
      </View>

      <View style={localStyles.listHeader}>
        <Text style={localStyles.listTitle}>Dernières déclarations</Text>
        <TouchableOpacity onPress={fetchDeclaredPalettes}>
...
const localStyles = StyleSheet.create({
  ...
  scanButtonText: { color: 'white', fontSize: 16, fontWeight: 'bold', letterSpacing: 1.5, textTransform: 'uppercase' },
